...
import json
from unified_query_handler import UnifiedQueryHandler
import logging
...
unified_handler = UnifiedQueryHandler()
logger.info("✅ Unified Query Handler initialized")
rag = PropBotRAG()
...
async def unified_search(request: ChatRequest):
    """
    Unified search across ALL collections (properties, rentals, crime, schools, etc.)
    """
    try:
        # Search across all collections
        results = unified_handler.get_property_with_context(request.query)
        ...
        search_history.append({
            ...
            "rentals_found": len(results['rentals'])
        })
        
        # Format response
        response = {
            "query": request.query,
            "properties": results['properties'][:10],  # Top 10 properties
            "rentals": results['rentals'][:10],  # Top 10 rentals
            "context": {
...
@app.get("/collections_status")
async def get_collections_status():
    """
    Check which ChromaDB collections are loaded
    """
    try:
        status = {
            "loaded_collections": list(unified_handler.collections.keys()),
            "total_collections": len(unified_handler.collections),
            "collection_details": {}
        }
        
        for name, collection in unified_handler.collections.items():
            status["collection_details"][name] = {
                "purpose": unified_handler.collection_map.get(name, "unknown"),
